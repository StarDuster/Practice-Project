...
#something else can be added to the log file name:
    read -p "enable keep-alive connection? 1 is for yes,2 is for no,default is yes:" IF_LONG
    if [ "$IF_LONG" = "" ];then
        IF_LONG=1
    fi
    if [ "$IF_LONG" = "1" ];then
    ...
    echo -e "testing...logging to $LOG_PATH/ \n"
        for (( i=1; i<$[PROCESS_NUM+1]; i=i+1 )); do
            ...
            ab -s 120 -r $LONG -n $REQ -c $CON $PROTOCOL://test.starduster.me/ | tee $LOG_NAME &
        done
    wait
    RUN_TIME=`cat $LOG_NAME | grep "Time taken" | awk '{print $5}'`
    COMPLETE_REQ=`cat $LOG_NAME | grep "Complete" | awk '{print $3}'`
